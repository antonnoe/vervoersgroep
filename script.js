document.addEventListener('DOMContentLoaded', function() {
    ...
    async function laadRitten() {
        ...
        try {
            ...
            const weergaveOrde = [ { key: 'vraag_lift', titel: 'Liftcentrale - Gevraagd' }, { key: 'aanbod_lift', titel: 'Liftcentrale - Aangeboden' }, { key: 'vraag_transport', titel: 'Transportcentrale - Gevraagd' }, { key: 'aanbod_transport', titel: 'Transportcentrale - Aangeboden' } ];
            ...
            weergaveOrde.forEach((groepInfo, index) => {
                ...
                if (groepData.length > 0) {
                    ...
                    groepData.forEach(rit => {
                        ...
                        let ritHTML = `<h4>${rit.van_plaats} &rarr; ${rit.naar_plaats}</h4><p><strong>Door:</strong> ${rit.naam_oproeper || 'Onbekend'}</p><p><strong>Datum:</strong> ${vertrekDatum}</p><p><strong>Details:</strong> ${rit.details || 'Geen'}</p><p><strong>Contact:</strong> <a href="mailto:${rit.contact_info}">${rit.contact_info}</a></p>`;
    ...
    vervoerForm.addEventListener('submit', async (event) => {
        ...
        // Genereer de unieke code hier, in de browser
        ...
        try {
            ...
            // Stuur de correcte token nu naar de e-mail functie
            await supabaseClient.functions.invoke('send-edit-link', {
                body: { 
                    toEmail: ritData.contact_info, 
                    editUrl: `${window.location.origin}${window.location.pathname}?edit=${ritData.edit_token}`,
