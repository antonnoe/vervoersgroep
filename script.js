document.addEventListener('DOMContentLoaded', function() {
    ...
    function parseCSV(text) {
        ...
        return rows.slice(1)
            // Filtert rijen die leeg zijn of geen ID hebben in de eerste kolom
            .filter(row => row && row.split(',')[0].trim() !== '')
            .map(row => {
    ...
    async function laadPagina() {
        ...
        try {
            // Voeg een timestamp toe om de cache te omzeilen
            ...
            // Sorteer op 'created_at' datum, nieuwste eerst
        ...
        if (data.length === 0) {
            ...
            data.forEach(rit => {
                ...
                ritDiv.innerHTML = `
                    ...
                    <p><strong>Details:</strong> ${rit.details || 'Ge-en'}</p>
    ...
    vervoerForm.addEventListener('submit', async (event) => {
        ...
        try {
            ...
            document.getElementById('modal-text').textContent = "Je oproep is succesvol geplaatst en zal binnen enkele minuten zichtbaar zijn. Een e-mail met een link om je oproep te beheren is onderweg.";
